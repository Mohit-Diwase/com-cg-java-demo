...
class Employees implements Serializable  {
	...
	Employees(String name, int age, double salary) {
		this.name = name;
		this.age = age;
		this.salary = salary;
	}
...
class EmpObjectSerializationDemo {
	...
	void writeData() {
		...
		Employees db[] = {
					new Employees("Sachin",25,12000.56), 
					new Employees("Rahul",24,12670.78),
					new Employees("Hritik",28,16000.89)
					};
	...
	void readData() {
		try(FileInputStream in = new FileInputStream("emp-obj.dat");
			...
			Employees e = (Employees) sin.readObject();
			e.showDetails();
			e = (Employees) sin.readObject();
			e.showDetails();
			e = (Employees) sin.readObject();	
			e.showDetails();
			sin.close();
